    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger className="w-full" asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className={`w-full justify-between items-start' ${className}`}
        >
      ...
      <PopoverContent className="w-[400px] p-0">
        <Command>
          <CommandInput placeholder="Pesquisar instituições..." className="h-9" onValueChange={handleSearch} />
          <CommandList>
            {loading ? (
              ...
              <CommandGroup>
                {organizations.map((org) => (
                  <CommandItem key={org.organizationId} value={org.name} onSelect={() => handleSelect(org)}>
                    {org.name}
                    <Check className={cn('ml-auto', value === org.name ? 'opacity-100' : 'opacity-0')} />
                  </CommandItem>
                ))}
              </CommandGroup>
            )}
